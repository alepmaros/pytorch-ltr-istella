# Taken from AllRank: https://github.com/allegro/allRank
...
def dcg(y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE):
    ...
    discounts = (torch.tensor(1, device=true_sorted_by_preds.device) / torch.log2(torch.arange(true_sorted_by_preds.shape[1], dtype=torch.float, device=true_sorted_by_preds.device) + 2.0))
